...
{
	// Clamp to 4 bits per channel and pack as 0xRGB (12-bit)
    uint16_t rgb12 = ((red & 0xF) << 8) | ((green & 0xF) << 4) | (blue & 0xF);

    // Two colors per 32-bit word
    uint32_t idx   = color >> 1;     // 0..7
    uint32_t even  = (color & 1u) == 0u;

    uint32_t w = hdmi_ctrl->PALETTE[idx];

    if (even) {
        // low 12 bits
        uint16_t other = (uint16_t)((w >> 16) & 0x00000FFF);  // keep high entry
        hdmi_ctrl->PALETTE[idx] = ((uint32_t)other << 16) | (uint32_t)rgb12;
    } else {
        // high 12 bits at [27:16]
        uint16_t other = (uint16_t)(w & 0x00000FFF);          // keep low entry
        hdmi_ctrl->PALETTE[idx] = ((uint32_t)rgb12 << 16) | (uint32_t)other;
    }
}
